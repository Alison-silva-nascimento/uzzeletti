...
function productCard(p){const badge=p.new?'Novidade':p.bestSeller?'Mais vendido':'';return `<article class="product-card category-${p.category} ${p.editorialBackground?'editorial-product':''}">${badge?`<span class="badge">${badge}</span>`:''}<a href="produto.html?id=${p.slug}&voltar=${encodeURIComponent('produtos.html'+savedCatalog())}" aria-label="Ver detalhes de ${p.name}"><div class="product-media" style="--product-image:url('/${encodeURI(p.images[0]).replace(/^\/+/, '')}')"><img src="${p.images[0]}" alt="${p.name}" loading="lazy"></div><div class="product-info"><div class="product-meta"><span>${categoryLabel(p.category)}</span></div><h3 class="product-name">${p.name}</h3>${Array.isArray(p.sizes)&&p.sizes.length?`<p class="product-sizes">Tamanhos: ${p.sizes.join(' · ')}</p>`:''}<div class="product-actions"><span class="product-price">${money(p.price)}</span><span class="product-cta">Ver detalhes →</span></div></div></a></article>`}
